...
typedef struct edge{
    ...
    vector<vector<bool>> wallMap;    // 壁判定 true で歩行おk

    vector<vector<Edge>> G;     // グラフ
    vector<bool> al_ch; // 到達済みのチェックポイントリスト
    vector<pair<int,int>> Lzi;    // 三叉路とL字座標
    vector<pair<int,int>> Konozi; // 行き止まり座標
...
pair<int,int> search_peret(int id,vector<int> &p_y,vector<int> &p_x,vector<vector<bool>> &peret){
    ...
    rep(i,4){
        if(0 <= ny+dp[i] && ny+dp[i] < height && 0 <= nx+dq[i] && nx+dq[i] < width){
            if(now_time <= 100 && rock_area[ny+dp[i]][nx+dq[i]])continue;
            if(peret[ny+dp[i]][nx+dq[i]])return {ny+dp[i],nx+dq[i]};
        }
        else if(nx+dq[i] == -1){
            if(now_time <= 100 && rock_area[ny+dp[i]][width-1])continue;
            if(peret[ny+dp[i]][width-1])return {ny+dp[i],width-1};
        }
        else if(nx+dq[i] == width){
            if(now_time <= 100 && rock_area[ny+dp[i]][0])continue;
...
void dfs(int p,int q,int prp,int prq,int n,int ct,int &ma,const vector<vector<bool>> &pnow,vector<vector<bool>> &pb,map<pair<int,int>,bool> &pac_exist,int dir,queue<pair<int,int>> que,int id){
    ...
    rep(i,4){
        ...
        int dirp = dp[(dir+2)%4];
        int dirq = dq[(dir+2)%4];
        if(now_time <= 100 && rock_area[np][nq])continue;
        if(n == 0)if(p + dirp == np && q + dirq == nq)continue;
        if(np == prp && nq == prq)continue;
        if(n >= 3)if(pac_exist[{np,nq}])continue;
...
void dfs_bigperet(int p,int q,int prp,int prq,int n,int ct,int &ma,const vector<vector<bool>> &pnow,vector<vector<bool>> &pb,map<pair<int,int>,bool> &pac_exist,int dir,queue<pair<int,int>> que,int id){
    ...
    rep(i,4){
        ...
        if(np == prp && nq == prq)continue;
        if(fie[np][nq] == '#')continue;
        if(0 <= np && np < height && 0 <= nq && nq < width){
            if(now_time <= 100 && rock_area[np][nq])continue;
            int point = ct;
            if(pb[np][nq])point += 20;
            ...
            if(now_time <= 100 && rock_area[np][width-1])continue;
            int point = ct;
            if(pb[np][width-1])point += 20;
            dfs_bigperet(np,width-1,p,q,n+1,point,ma,pnow,pb,pac_exist,dir,nque,id);
        }
        else if(nq == width){
            if(now_time <= 100 && rock_area[np][0])continue;
            int point = ct;
            if(pb[np][0])point += 20;
            dfs_bigperet(np,0,p,q,n+1,point,ma,pnow,pb,pac_exist,dir,nque,id);
        }
    }
...
pair<int,int> check_enemy_pos(int id,vector<int> &p_y,vector<int> &p_x,vector<string> &p_type,vector<int> &ene_pac_id){
    // 8マス圏内に敵がいたら敵の位置を返す
    int sz = ene_pac_id.size();
    rep(i,sz){
        int tmp = get_distance(id,ene_pac_id[i],p_y,p_x);
        if(tmp <= 8){
            return {p_y[ene_pac_id[i]],p_x[ene_pac_id[i]]};
        }
    }
    return {-1,-1};
}
...
void GraphMake(int p,int q){
    // グラフ作成とついでにチェックポイントリストも生成
    if(!wallMap[p][q])return;
    int nowId = p*width + q;
    int ct = 0;
    pair<int,int> a;
    pair<int,int> b;
    rep(k,4){
        int np = (p + dp[k] + height) % height;
        int nq = (q + dq[k] + width) % width;
        if(wallMap[np][nq]){
            ct++;
            if(ct == 1){
                a = {np,nq};
        ...
        if(wallMap[np][nq]){
            int toId = np * width + nq;
            G[nowId].emplace_back(toId,1);
        }
    }
    if(ct == 3){
        Konozi.push_back({p,q});
        al_ch[nowId] = false;
    }
    if(ct == 2){
        if(abs(a.F - b.S) == 1 && abs(a.S - b.S) == 1){
            Lzi.push_back({p,q});
            al_ch[nowId] = false;
        }
    }
...
int main()
{
    ...
    wallMap.resize(height+1,vector<bool>(width+1,false));
    G.resize(height*width+1);
    al_ch.resize(height*width,true);
    int check_ct = 0;   // チェックポイント踏破数

    int quop[5] = {height/4,height/4,height*3/4,height*3/4,height/2};    // 4分割した時の中心p
    int quoq[5] = {width/4,width*3/4,width/4,width*3/4,width/2};        // 4分割した時の中心q
    int range_quo = max(height/4,width/4);      // 各中心からのおおよその半径
    ...
    rep(i,height)rep(j,width)if(fie[i][j] == ' ')wallMap[i][j] = true;
    rep(i,height)rep(j,width){
        if(!wallMap[i][j])continue;
        GraphMake(i,j);
    }
    ...
    */
    ...
    while (1) {
        ...
        rep(i,lim){
            ...
            int nowpos = ny*width + nx;

            vector<LL> minroute = dijkstra(height*width,G,nowpos);
            pair<int,int> pos_dist;

            int bigct = -1;
            int lenbig = 100000;
            pair<int,int> posbig;
            for(auto t : big_peret){
                if(big_peret_bool[t.F][t.S]){
                    int pospb = t.F*width + t.S;
                    if(lenbig > minroute[pospb]){
                        bigct = 0;
                        lenbig = minroute[pospb];
                        posbig = t;
                    }
                }
            }                        
...
/*
            vector<LL> minroute = dijkstra(height*width,G,nowpos);
            pair<int,int> pos_dist;
            if(check_ct < Lzi.size()-2){
                int pt = 15000;
                for(auto t : Lzi){
                    int distid = t.F*height + t.S;
                    if(al_ch[distid])continue;
                    int now_quoid = quop[my_pac_id[i]]*width + quoq[my_pac_id[i]];
                    int dis_tmp_p = t.F;
                    int dis_tmp_q = t.S;
                    int dis_quop = now_quoid % height;
                    int dis_quoq = now_quoid % width;
                    int point_dis = minroute[distid];
                    if(max(abs(dis_tmp_p - dis_quop),abs(dis_tmp_q - dis_quoq)) > range_quo)point_dis += 10;
                    if(point_dis < pt){
                        pt = point_dis;
                        pos_dist = t;
                    }
                ...
                for(auto t : Konozi){
                    ...
                    int dis_quoq = now_quoid % width;
                    int point_dis = minroute[distid];
                    if(max(abs(dis_tmp_p - dis_quop),abs(dis_tmp_q - dis_quoq)) > range_quo)point_dis += 50;
                    if(point_dis < pt){
                        pt = point_dis;
                        pos_dist = t;
                    }                         
...
*/
            if(bigct == 0){
                    p_destination[my_pac_id[i]] = posbig;
        ...
        ///////   エンドフェイズ
        int bigPnum = big_peret.size();
        bool ok = false;
        if(bigPnum > 0 || myScore <= 100)ok = true;
        if(ok && now_time % 15 == 1){
            // 100回に一回SPEED
            rep(i,lim){
                if(p_abicool[my_pac_id[i]] == 0){
                    speed(my_pac_id[i]);
                    p_received[my_pac_id[i]] = false;
                }
                else{
